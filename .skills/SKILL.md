...
1. **HTML Headers** (generated by `scripts/build_html.py`):
   ...
   - **Source**: the `.html`/`.js` files next to them in `src/network/html/`
     (`scripts/build_html.py` walks `src/`, see `scripts/build_html.py:5`)
   - **Triggered**: During PlatformIO `pre:` build step
   - **To modify**: Edit the source HTML in `src/network/html/`, not generated headers
...
`docs/power-management.md`, `docs/sleep-screen.md`. Extend one before adding
another.
