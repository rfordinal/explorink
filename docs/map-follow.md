...
re-anchor. `Nearby -> View on map` re-anchors on a POI that can be kilometres
away, so the next fix moves the marker across the panel. The feature did not
create the bug; it made the far jump ordinary.
