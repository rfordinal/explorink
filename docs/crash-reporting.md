...
Both watchdogs are armed in the effective config
(`CONFIG_ESP_INT_WDT_TIMEOUT_MS=300`, `CONFIG_ESP_TASK_WDT_PANIC=y`,
`CONFIG_ESP_TASK_WDT_TIMEOUT_S=5`). A five-second freeze
...
**Where to read "the effective config", and where not to.** Check
`~/.platformio/packages/framework-arduinoespressif32-libs/esp32s3/sdkconfig` --
the framework's prebuilt config, on disk for anyone who has built once. The
firmware repo's own `sdkconfig.defaults` states the same values, but **do not
cite that one**: it is generated by `custom_sdkconfig` (`platformio.ini`,
`[base]`) and gitignored (`.gitignore:35`), so a fresh clone of this repo does
not have it and a reader cannot check a claim that points at it. Corrected
2026-09-01, after this doc was first written citing exactly that file.

**Corrected 2026-09-05: that file is shared mutable state, not a stable
artifact.** `framework-arduinoespressif32-libs/` holds one subtree per chip, and
a build rewrites the subtree for the chip it targets, so its contents are
whatever the last build on this machine left there -- any session, any branch,
any worktree. Both subtrees were observed swapping which one looked freshly
built inside one hour. It is still the right place to look, and still better
than the gitignored generated file, but a claim taken from it **names the date
it was read and says which environment had last built**. When the claim is
load-bearing, read the linked `firmware.elf` instead: the binary cannot drift.
See [`build-environments.md`](build-environments.md), "Where `custom_sdkconfig`
actually lands".
